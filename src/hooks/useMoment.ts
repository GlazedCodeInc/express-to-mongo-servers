...
interface DateFormatType {
    ...
    yyyy년mm월: string
    yyyy년mm월dd일: string
    'yyyy년mm월dd일 요일': string
    'mm월dd일 요일': string
    'yyyy년mm월dd일 hh:mm': string
    yy년mm월: string
    yy년mm월dd일: string
    'yy년mm월dd일 hh:mm': string
}

type DateFormatKey = keyof DateFormatType
...
export function useMoment(dateParam: string | Date) {
    ...
    return {
        format: (typeParam: DateFormatKey): string => {
            ...
            if (type === 'yyyy.mm.dd 요일') {
                return `${year}.${formattedMonth}.${formattedDay} ${weekDay}요일`
            }
            ...
            if (type === 'yyyy년mm월dd일 요일') {
                return `${year}년 ${formattedMonth}월 ${formattedDay}일 ${weekDay}요일`
            }

            if (type === 'mm월dd일 요일') {
                return `${formattedMonth}월 ${formattedDay}일 ${weekDay}요일`
            }
            ...
            if (type === 'yy년mm월dd일 hh:mm') {
                return `${year
                    .toString()
                    .slice(-2)}년 ${formattedMonth}월 ${formattedDay}일 ${period} ${adjustedHours}:${formattedMinutes}`
            }

            // 기본 반환값 (모든 경우를 처리했지만 타입 안전성을 위해 추가)
            return `${year}.${formattedMonth}.${formattedDay}`
        },
